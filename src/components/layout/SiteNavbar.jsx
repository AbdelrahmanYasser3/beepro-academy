...
const SiteNavbar = ({ onAuthClick, hideTeacherSignup = false }) => {
    ...
    <>
      {!hideTeacherSignup && (
        <button
          ...
          onClick={() => handleAuthClick("register", "teacher")}
        >
          {t("nav.teach")}
        </button>
      )}
  ...
  return (
    <header
      className={`site-navbar fixed top-0 inset-x-0 z-[1000] w-full max-w-[100vw] overflow-visible transition-all duration-300 ${
        scrolled
          ? "bg-[rgba(0,4,40,0.98)] shadow-[0_2px_20px_rgba(0,0,0,0.3)]"
          : "bg-[rgba(0,4,40,0.95)] shadow-[0_2px_20px_rgba(0,0,0,0.3)]"
      } backdrop-blur-xl`}
    >
      <nav
        className={`mx-auto flex w-full max-w-full min-w-0 items-center justify-between gap-2 overflow-visible px-3 sm:px-4 md:gap-4 md:px-6 lg:px-10 ${navBarHeight(scrolled)}`}
      >
        {/* Logo */}
        <Link
          to="/"
        ...
        <div className="flex shrink-0 items-center gap-1 sm:gap-2">
          ...
          {isAuthenticated ? (
            ...
            >
              ...
              {isProfileOpen && (
                <div className="absolute end-0 top-[calc(100%+10px)] z-[1100] mt-1 w-56 max-w-[calc(100vw-2rem)] animate-slide-down overflow-visible rounded-xl border border-[#00d9ff]/25 bg-[rgba(0,12,48,0.98)] shadow-[0_12px_40px_rgba(0,0,0,0.45)]">
