...
const PaymentCheckout = () => {
  ...
  const { formatCoursePrice, currencyCode, convertFromUsd, currency } =
    useCurrency();
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const methods = await paymentService.getCoursePaymentMethods(
          id,
      ...
      <div className="min-h-screen pt-24 container-custom">
        <div className="card card-body max-w-2xl mx-auto text-center">
          <h2 className="text-2xl font-bold mb-3">
            {t("paymentCheckout.courseIsNotAvailable")}
          </h2>
      ...
      <div className="container-custom max-w-5xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 min-w-0">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <div className="mt-2 text-sm text-secondary-700 dark:text-secondary-300 space-y-1">
                              {formatDetails(method.payment_details).map(
                                (item) => (
                                  <p key={item.key}>
                ...
                <div>
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="label">
                        {t("paymentCheckout.transactionReference")}
                      </label>
                      <input
                        type="text"
                  ...
                  <div className="mt-4">
                    ...
                    <input
                      ...
                      className="input"
                      placeholder={t(
                        "paymentCheckout.enterSenderPhoneaccountDetails",
                      )}
                      required
                    />
                  </div>
                  ...
                  <div className="mt-4">
                    <label className="label">
                      {t("paymentCheckout.additionalNotes")}
                    </label>
                    <textarea
                      ...
                      placeholder={t(
                        "paymentCheckout.anyAdditionalPaymentNotes",
                      )}
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <label className="border-2 border-dashed rounded-xl p-6 text-center block cursor-pointer hover:border-primary-400 transition-colors">
                    ...
                    <p className="font-medium">
                      {screenshotFile
                        ? screenshotFile.name
                        : t("paymentCheckout.choosePaymentScreenshot")}
                    </p>
                    <input
                      type="file"
                      accept="image/*"
                ...
                <div className="flex gap-3">
                  <button
                    type="submit"
                    disabled={
                      isSubmitting || paymentMethods.length === 0 || !isStudent
                    }
          ...
          <div className="card card-body h-fit">
            <h3 className="text-lg font-bold mb-3">
              {t("paymentCheckout.orderSummary")}
            </h3>
            <div className="space-y-2 text-sm">
              <p>
                <span className="font-medium">
                  {t("paymentCheckout.course")}
                </span>{" "}
                {course.title}
              </p>
              <p>
                <span className="font-medium">
                  {t("paymentCheckout.price")}
                </span>{" "}
                {formatCoursePrice(course.price || 0).full}
              </p>
              <p>
                <span className="font-medium">
                  {t("paymentCheckout.status")}
                </span>{" "}
                {t("paymentCheckout.awaitingPaymentProof")}
              </p>
            </div>
          </div>
        </div>
