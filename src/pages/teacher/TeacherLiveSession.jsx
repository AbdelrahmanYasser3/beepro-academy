...
const TeacherLiveSession = () => {
  ...
  const liveSessions = useMemo(
    () =>
      sessions.filter(
        (session) =>
          isJitsiMeeting(session) ||
          isExternalGoogleMeet(session) ||
  ...
  const sendSessionLinkToStudents = async (session, options = {}) => {
    ...
    try {
      const sessionTitle =
        session.title || t("teacherLiveSession.liveSession_38");
      const platformLabel =
        shareInfo.platform === "google_meet" ? "Google Meet" : "Jitsi";
      let notifyResult = { count: 0 };
      ...
      if (!silent) {
        setError(
          err.message || t("teacherLiveSession.failedToSendLinkToStudents"),
        );
      }
      throw err;
    } finally {
  ...
  const startLiveSession = async (startNow = true, titleOverride = "") => {
    ...
    const sessionTitle =
      ...
      t("teacherLiveSession.liveSession_37");
    const scheduledAt =
      form.scheduled_at || new Date().toISOString().slice(0, 16);
    const isManualGoogleMeet =
      sessionPlatform === "google_meet" && googleMeetLinkMode === "manual";
    ...
    if (sessionPlatform === "google_meet" && isManualGoogleMeet) {
      ...
      if (!isValidGoogleMeetLink(meetLink)) {
        setError(t("teacherLiveSession.pleaseEnterAValidGoogleMeetLin"));
        return;
      }
    } else if (
    ...
    try {
      ...
      const notifyResult = await sendSessionLinkToStudents(resolvedMeeting, {
        silent: true,
      });
      const platformLabel =
        sessionPlatform === "google_meet" ? "Google Meet" : "Jitsi";
      ...
      const refreshed = await meetingService.getMeetingsByCourse(
        selectedCourseId,
        { instructorView: true },
      );
  ...
  const renderShareLinkCard = (session, compact = false) => {
    ...
    return (
      <div
        className={`rounded-lg border border-green-200 dark:border-green-800 bg-green-50/80 dark:bg-green-900/20 ${compact ? "p-3" : "p-4"} space-y-3`}
      >
  ...
  const renderPlatformPicker = () => (
    <div className="space-y-3">
      <label className="label">
        {t("teacherLiveSession.sessionPlatform_35")}
      </label>
      <div
        className="grid grid-cols-2 gap-2"
        role="group"
        aria-label={t("teacherLiveSession.sessionPlatform")}
      >
        <button
          type="button"
      ...
      {sessionPlatform === "google_meet" && (
        <>
          <div
            className="grid grid-cols-2 gap-2"
            role="group"
            aria-label={t("teacherLiveSession.googleMeetLinkSource")}
          >
            <button
              type="button"
          ...
          {googleMeetLinkMode === "manual" && (
            <div>
              <label className="label">
                {t("teacherLiveSession.googleMeetLink")}
              </label>
              <input
                className="input"
                ...
                onChange={(e) =>
                  setForm((prev) => ({
                    ...prev,
                    manual_meet_link: e.target.value,
                  }))
                }
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            <Link
              ...
              className="inline-flex items-center gap-2 text-sm text-secondary-500 hover:text-primary-500 mb-2"
            >
          ...
          <>
            ...
            <div className="card card-body space-y-4">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  <label className="label">
                    {t("teacherLiveSession.descriptionOptional")}
                  </label>
                  <textarea
                ...
                <div>
                  <label className="label">
                    {t("teacherLiveSession.scheduledTime")}
                  </label>
                  <input
                    type="datetime-local"
                    className="input"
                ...
                <div>
                  <label className="label">
                    {t("teacherLiveSession.durationMinutes")}
                  </label>
                  <input
                    type="number"
                    min={15}
                            ...
                            onClick={() =>
                              setActiveSession(
                                isGoogleMeetSession(session)
                                  ...
                                  : {
                                      ...session,
                                      jitsi_room_name: resolveJitsiRoomName(
                                        session,
                                        selectedCourseId,
                                      ),
                                    },
                          ...
                          <div className="flex flex-col sm:flex-row gap-2">
                            ...
                            <div className="flex flex-wrap gap-2">
                              ...
                              <button
                                ...
                                disabled={sendingLink}
                                onClick={() =>
                                  sendSessionLinkToStudents(session)
                                }
                              >
                                <FiSend className="w-4 h-4" />
