...
const Dashboard = () => {
  ...
  const [isLoadingTeacherPayments, setIsLoadingTeacherPayments] =
    useState(false);
    ...
    return (studentEnrollments || [])
      ...
      .map((enrollment) => ({
        ...
        descriptionEn:
          ...
          "",
        thumbnail:
          enrollment.course.thumbnail_url || "/assets/hero-background.png",
        ...
        instructor: {
          ...
          avatar:
            enrollment.course.instructor?.avatar_url || "/assets/abdullah1.jpg",
        },
      }));
  }, [studentEnrollments]);

  const completedCourses = enrolledCourses.filter(
    (course) => user?.progress?.[course.id] === 100,
  );
  ...
  const stats = [
    ...
    },
    {
      ...
      value: enrolledCourses
        .reduce(
          (acc, course) =>
            acc + ((user?.progress?.[course.id] || 0) * course.duration) / 100,
          0,
        )
        .toFixed(0),
  ...
  useEffect(() => {
    const fetchTeacherPayments = async () => {
      ...
      try {
        const submissions =
          await paymentService.getInstructorPaymentSubmissions(user.id);
  ...
  const handleStudentNotificationJoin = async (notification) => {
    const joinPath = notification.action_url
      ...
      : notification.course_id
        ? `/courses/${notification.course_id}/learn?session=live`
        : null;
  ...
  const handlePaymentReview = async (submissionId, action) => {
    if (!user?.id) {
      setPaymentError(t("dashboard.youMustBeSignedInToReviewPayme"));
      return;
    }
    ...
    try {
      ...
      setPaymentSuccess(
        action === "approve"
          ? t("dashboardExtra.paymentApprovedSuccess")
          : t("dashboardExtra.paymentRejectedSuccess"),
      );
      ...
      setPaymentError(error.message || t("dashboardExtra.paymentUpdateFailed"));
      paymentFeedbackRef.current?.scrollIntoView({
        behavior: "smooth",
        block: "nearest",
      });
  ...
  const handlePasswordSave = async (event) => {
    ...
    try {
      ...
      setPasswordStatus({
        type: "success",
        message: t("profileSettings.passwordUpdated"),
      });
    } catch (error) {
      setPasswordStatus({
        type: "error",
        message: error.message || t("profileSettings.passwordUpdateFailed"),
      });
    } finally {
  ...
  const sidebarLinks = [
      ...
      ? [
          {
            id: "teacher",
            icon: FiPlusCircle,
            label: t("dashboardExtra.manageCourses"),
            route: "/dashboard?tab=teacher",
            kind: "tab",
          },
        ]
      : []),
      ...
      ? [
          {
            id: "admin",
            icon: FiShield,
            label: t("dashboardExtra.adminPanel"),
      ...
      ? [
          {
            id: "homework",
            icon: FiBookOpen,
            label: t("homework.navLabel"),
            route: "/dashboard?tab=homework",
            kind: "tab",
          },
        ]
      : []),
    ...
    },
    {
      ...
      route: "/dashboard?tab=certificates",
      kind: "tab",
    },
  ];

  const isSidebarActive = (link) => {
    if (link.kind === "route") {
      ...
      const [routePath] = link.route.split("?");
      return (
    ...
    const [routePath, routeQuery = ""] = link.route.split("?");
      ...
      <div className="container-custom">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {stats.map((stat, index) => (
            <div key={index} className="card card-body flex items-center gap-4">
              <div
                className={`w-12 h-12 rounded-xl bg-gradient-to-br ${stat.color} flex items-center justify-center shrink-0`}
              >
        ...
        <div className="grid lg:grid-cols-4 gap-8">
            ...
            <div className="card overflow-hidden">
              <div className="flex lg:flex-col overflow-x-auto lg:overflow-x-visible">
                {sidebarLinks.map((link) => {
                  const active = isSidebarActive(link);
                  const baseClass =
                    "flex-shrink-0 lg:w-full flex items-center gap-3 px-4 py-4 transition-colors whitespace-nowrap lg:whitespace-normal";
                  const activeClass =
          ...
          <div className="lg:col-span-3 min-w-0">
                      ...
                      <div className="card card-body">
                        ...
                        <div className="space-y-3">
                          {studentNotifications.map((notification) => {
                            const jitsiMatch =
                              notification.message?.match(/https:\/\/[^\s]+/i);
                          ...
                          <table className="w-full">
                            <thead>
                              <tr className="border-b border-gray-200 dark:border-gray-700">
                                <th className="text-start py-2 px-2">
                                  {t("dashboardExtra.date")}
                                </th>
                                <th className="text-start py-2 px-2">
                                  {t("dashboardExtra.course")}
                                </th>
                                <th className="text-start py-2 px-2">
                                  {t("dashboardExtra.amount")}
                                </th>
                                <th className="text-start py-2 px-2">
                                  {t("dashboardExtra.method")}
                                </th>
                                <th className="text-start py-2 px-2">
                                  {t("dashboardExtra.reference")}
                                </th>
                                <th className="text-start py-2 px-2">
                                ...
                                <th className="text-start py-2 px-2">
                                  {t("dashboardExtra.reviewNotes")}
                                </th>
                                <th className="text-start py-2 px-2">
                                  {t("dashboardExtra.proof")}
                                </th>
                              </tr>
                            </thead>
                            <tbody>
                    ...
                    <div className="grid md:grid-cols-3 gap-4">
                      <div className="card card-body text-center">
                        <div className="text-2xl font-bold text-yellow-600">
                          {
                            teacherPaymentSubmissions.filter(
                              (s) => s.status === "pending",
                            ).length
                          }
                        </div>
                        <div className="text-sm text-gray-500">
                          {t("dashboardExtra.pending")}
                        </div>
                      </div>
                      <div className="card card-body text-center">
                      ...
                      <div className="card card-body text-center">
                        ...
                        <div className="text-sm text-gray-500">
                          {t("dashboardExtra.approvedTotal")}
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full">
                          ...
                          <tbody>
                            ...
                            {!isLoadingTeacherPayments &&
                              teacherPaymentSubmissions.map((submission) => (
                                <tr
                                  key={submission.id}
                                  className="border-b border-gray-100 dark:border-gray-800"
                                >
                                  ...
                                  <td className="py-3 px-4">
                                    {formatPaymentAmount(submission)}
                                  </td>
                                  <td className="py-3 px-4">
                                    <span
                                      className={`px-2 py-1 text-xs rounded-full ${
                            ...
                            {!isLoadingTeacherPayments &&
                              teacherPaymentSubmissions.length === 0 && (
                                <tr>
                                  <td
                                    colSpan={5}
                                    className="py-6 text-center text-gray-500"
                    ...
                    {myCourses.length === 0 ? (
                      ...
                      <>
                        <label className="label">
                          {t("dashboardExtra.selectCourse")}
                        </label>
                        <select
                          className="input mb-4 max-w-md"
                          value={teacherChatCourseId || myCourses[0]?.id || ""}
                          onChange={(e) =>
                            setTeacherChatCourseId(e.target.value)
                          }
                        >
                      ...
                      {isLoadingCourses ? (
                        ...
                        <div className="space-y-4">
                          {myCourses.map((course) => (
                            <div
                              key={course.id}
                              className="flex items-center gap-4 p-4 bg-secondary-50 dark:bg-dark-border rounded-xl"
                            >
                              <img
                                src={
                                  course.thumbnail_url ||
                  ...
                  <div>
                    ...
                    <div className="grid md:grid-cols-3 gap-6 mb-8">
                      <div className="card card-body bg-gradient-to-br from-green-500 to-teal-600 text-white">
                        ...
                        <div className="text-white/80">
                          {t("dashboardExtra.totalPayments")}
                        </div>
                      </div>
                      <div className="card card-body bg-gradient-to-br from-blue-500 to-indigo-600 text-white">
                        ...
                        <div className="text-white/80">
                          {t("dashboardExtra.pendingPayments")}
                        </div>
                      </div>
                      <div className="card card-body bg-gradient-to-br from-purple-500 to-pink-600 text-white">
                        ...
                        <div className="text-white/80">
                          {t("dashboardExtra.activePaymentMethods")}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="card card-body mb-8">
                      ...
                      >
                        <div>
                          <label className="label">
                            {t("dashboardExtra.paymentType")}
                          </label>
                          <select
                            value={paymentForm.payment_type}
                            onChange={(e) => {
                              const selected = PAYMENT_TYPES.find(
                              ...
                              setPaymentForm((prev) => ({
                                ...
                                display_name:
                                  selected?.label || prev.display_name,
                              }));
                            }}
                            className="input"
                          >
                        ...
                        <div className="md:col-span-2">
                          ...
                          <textarea
                            ...
                            placeholder={t(
                              "dashboard.example2010xxxxxxxOrPaypaypalc",
                            )}
                            required
                          />
                        </div>
                        ...
                        <div className="md:col-span-2">
                          <label className="label">
                            {t("dashboardExtra.paymentInstructions")}
                          </label>
                          <textarea
                            className="input min-h-[100px]"
                            ...
                            placeholder={t(
                              "dashboardExtra.instructionsPlaceholder",
                            )}
                          />
                        </div>
                            ...
                            {!isLoadingPayments &&
                              paymentSubmissions.map((submission) => (
                                ...
                                >
                                  ...
                                  <td className="py-3 px-4">
                                    {submission.payment_method?.display_name ||
                                      submission.payment_method?.payment_type}
                                  </td>
                                  <td className="py-3 px-4">
                                    <span
                                      className={`px-2 py-1 text-xs rounded-full ${
                        ...
                        <div key={course.id} className="card overflow-hidden">
                          <div className="bg-gradient-to-br from-yellow-500 to-orange-600 p-6 text-white text-center">
                            ...
                            <h3 className="text-xl font-bold">
                              {t("dashboardExtra.certificateOfCompletion")}
                            </h3>
                          </div>
                          <div className="p-6">
            ...
            {activeTab === "homework" && !isAdmin && <HomeworkPanel />}
