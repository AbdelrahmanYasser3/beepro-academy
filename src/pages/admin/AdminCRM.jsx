...
const AdminCRM = () => {
  ...
  const fetchContacts = async () => {
    ...
    try {
      ...
      setError(fetchError.message || t("adminCRM.unableToLoadCrmData"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredContacts = useMemo(() => {
    ...
    return contacts.filter((contact) => {
      const matchesStatus =
        statusFilter === "all" || contact.payment_status === statusFilter;
      const text = [
        contact.full_name,
        contact.email,
  ...
  const paidCount = contacts.filter(
    (contact) => contact.payment_status === "have payment",
  ).length;
  const unpaidCount = contacts.filter(
    (contact) => contact.payment_status === "without payment",
  ).length;
