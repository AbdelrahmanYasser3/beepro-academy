...
const ArticleSchedulePanel = ({ courses = [], onPostGenerated }) => {
  ...
  useEffect(() => {
    ...
    articleScheduleService
      .processDueSchedules({ courses, authorId: user?.id })
      .then((results) => {
        if (results.length > 0) {
          loadSchedules();
          onPostGenerated?.();
  ...
  const runSchedule = async (schedule) => {
    ...
    try {
      ...
      setError(
        err?.message || t("articleSchedulePanel.articleGenerationFailed"),
      );
      await loadSchedules();
    } finally {
  ...
  const cancelSchedule = async (scheduleId) => {
    try {
      await articleScheduleService.updateSchedule(scheduleId, {
        status: "cancelled",
      });
      setSchedules((current) =>
        current.map((row) =>
          row.id === scheduleId ? { ...row, status: "cancelled" } : row,
        ),
      );
    } catch (err) {
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <div className="bepro-card p-5">
          <p className="text-white/60 text-sm mb-1">
            {t("articleSchedulePanel.upcoming")}
          </p>
          <p className="text-3xl font-bold text-white">{stats.pending}</p>
        </div>
        <div className="bepro-card p-5">
          <p className="text-white/60 text-sm mb-1">
            {t("articleSchedulePanel.generated")}
          </p>
          <p className="text-3xl font-bold text-green-400">{stats.ready}</p>
        </div>
        <div className="bepro-card p-5">
          <p className="text-white/60 text-sm mb-1">
            {t("articleSchedulePanel.failed")}
          </p>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_minmax(280px,380px)] gap-6 items-start">
        <form onSubmit={addSchedule} className="bepro-card p-6 space-y-4">
          ...
          <label className="block">
            <span className="text-white font-bold mb-2 block">
              {t("articleSchedulePanel.topicOptional")}
            </span>
            <input
              value={form.title_hint}
              onChange={(e) => updateField("title_hint", e.target.value)}
              placeholder={t(
                "articleSchedulePanel.egBeginnerGuideToTechnicalAnal",
              )}
              className="form-input form-input-glass"
            />
          </label>

          <label className="block">
            <span className="text-white font-bold mb-2 block">
              {t("articleSchedulePanel.relatedCourse")}
            </span>
            <select
            ...
            >
              <option className="bg-[#000428]" value="">
                {t("articleSchedulePanel.autoSelect")}
              </option>
              {courses.map((course) => (
                <option
                  className="bg-[#000428]"
          ...
          <label className="block">
            <span className="text-white font-bold mb-2 block">
              {t("articleSchedulePanel.scheduledAt")}
            </span>
            <input
              required
              type="datetime-local"
          ...
          <label className="block">
            <span className="text-white font-bold mb-2 block">
              {t("articleSchedulePanel.extraAiInstructions")}
            </span>
            <textarea
              rows="3"
              ...
              placeholder={t(
                "articleSchedulePanel.focusOnPracticalBenefitsForStu",
              )}
              className="form-textarea form-textarea-glass"
            />
          </label>
          ...
          <button
            type="submit"
            disabled={isSaving}
            className="bepro-btn-primary w-full justify-center min-h-[48px]"
          >
            {isSaving ? (
                ...
                <thead>
                  <tr className="text-white/60 border-b border-white/10">
                    <th className="text-start py-3 px-2 font-semibold">
                      {t("articleSchedulePanel.schedule")}
                    </th>
                    <th className="text-start py-3 px-2 font-semibold">
                      {t("articleSchedulePanel.topicCourse")}
                    </th>
                    <th className="text-start py-3 px-2 font-semibold">
                      {t("dashboardExtra.status")}
                    </th>
                    <th className="text-start py-3 px-2 font-semibold">
                      {t("articleSchedulePanel.publish")}
                    </th>
                    <th className="text-start py-3 px-2 font-semibold">
                      {t("articleSchedulePanel.article")}
                    </th>
                    <th className="text-end py-3 px-2 font-semibold">
                      {t("articleSchedulePanel.actions")}
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {schedules.map((row) => {
                    ...
                    const isProcessing =
                      processingId === row.id || row.status === "generating";
                      ...
                      >
                        <td className="py-4 px-2 align-top">
                          <div className="flex items-start gap-2">
                            <FiClock
                              className={`w-4 h-4 mt-0.5 shrink-0 ${isDue ? "text-amber-300" : "text-white/40"}`}
                            />
                            <div>
                              <p className="font-medium">
                                {formatScheduleDate(row.scheduled_at, language)}
                              </p>
                              {isDue && (
                                <span className="text-xs text-amber-300">
                                  {t("articleSchedulePanel.dueNow")}
                                </span>
                              )}
                            </div>
                          </div>
                        ...
                        <td className="py-4 px-2 align-top">
                          <p className="font-medium">
                            {row.title_hint ||
                              row.course?.title ||
                              t("articleSchedulePanel.generalTopic")}
                          </p>
                        ...
                        <td className="py-4 px-2 align-top">
                          <span
                            className={`inline-flex px-2.5 py-1 rounded-full text-xs font-semibold ${STATUS_COLORS[row.status] || STATUS_COLORS.pending}`}
                          >
                            {t(statusLabel)}
                          </span>
                          {row.error_message && (
                        ...
                        <td className="py-4 px-2 align-top">
                          {row.blog_post ? (
                            ...
                            >
                              ...
                              {row.blog_post.title?.slice(0, 28) ||
